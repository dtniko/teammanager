...
const AthleteDetail = () => {
    ...
    const getDocumentsByType = () => {
        const types = ['medical_certificate', 'payment'];
        return types.map(type => {
            const docs = documents.filter(d => d.document_type === type);
            let currentDoc = null;
            if (seasonFilter) {
                currentDoc = docs.find(d => d.season_id === parseInt(seasonFilter)) || docs[0] || null;
            } else {
    ...
    const openUploadForm = (documentType = 'other') => {
        ...
        <div className="space-y-6">
            ...
            {activeTab === 'documents' && (
                <div className="space-y-6">
                    ...
                    {/* 2 Fixed Document Type Cards */}
                    {(() => {
                        const docTypes = getDocumentsByType();
                        return docTypes.map(({ type, label, currentDoc }) => (
                            <div key={type} className="bg-white shadow rounded-lg overflow-hidden">
                                ...
                                {currentDoc ? (
                                    <div className="p-6">
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <h5 className="text-sm font-medium text-gray-900">{currentDoc.title}</h5>
                                                <p className="text-xs text-gray-400">
                                                    Caricato il {format(new Date(currentDoc.created_at), 'dd/MM/yyyy HH:mm')}
                                                </p>

                                                {currentDoc.expiry_date && (
                                                    <div className="mt-2">
                                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getDocumentStatusColor(currentDoc)}`}>
                                                            {currentDoc.expired ? (
                                                                <>
                                                                    <AlertTriangle className="h-3 w-3 mr-1" />
                                                                    Scaduto il {format(new Date(currentDoc.expiry_date), 'dd/MM/yyyy')}
                                                                </>
                                                            ) : currentDoc.expiring_soon ? (
                                                                <>
                                                                    <AlertTriangle className="h-3 w-3 mr-1" />
                                                                    Scade il {format(new Date(currentDoc.expiry_date), 'dd/MM/yyyy')}
                                                                </>
                                                            ) : (
                                                                <>
                                                                    <CheckCircle className="h-3 w-3 mr-1" />
                                                                    Valido fino al {format(new Date(currentDoc.expiry_date), 'dd/MM/yyyy')}
                                                                </>
                                                            )}
                                                        </span>
                                                    </div>
                                                ...
                                                {currentDoc.notes && (
                                                    <p className="mt-2 text-sm text-gray-600">{currentDoc.notes}</p>
                                                )}
                                            </div>

                                            <div className="flex items-center space-x-2">
                                                <button
                                                    onClick={() => handleDocumentDownload(currentDoc.id, currentDoc.title)}
                                                ...
                                                {canUploadDocuments() && (
                                                    <button
                                                        onClick={() => handleDocumentDelete(currentDoc.id)}
                                    ...
                                    <div className="text-center py-8">
                                        ...
                                        <h4 className="mt-2 text-sm font-medium text-gray-500">Nessun documento caricato</h4>
                                        {canUploadDocuments() && (
                                            <div className="mt-4">
                                                <button
                                                    type="button"
                                                    onClick={() => openUploadForm(type)}
                                                    className="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700"
                                                >
